...
def get_adsorption_energy(metal, oxidation_state, adsorbate, facet=None):
    os_facet_match = {2:'facet',3:'012',4:'110',5:'18'}
    if facet is None:
...
def get_metal_potential(filename, metal_symbol):
    with open(filename, 'r') as f:
        lines = f.readlines()
    for line in lines:
        ...
        if ' O ' in line:
            O_potential = metal_potential = float(line.split()[2])

    return metal_potential* O_potential
